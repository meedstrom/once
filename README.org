...
2. It will almost never matter that =once-load= only calls a thing once, because a given Elisp file rarely loads twice anyway.  But it feels safer and easier to reason about, in my opinion: a given invocation cannot ever result in more than one call of the given function.
   ...
   There is no =remove-hook= equivalent for =eval-after-load=, and it is not easy to implement.  So the next best thing is to default to =once-load= everywhere.  Just in case.

* Similar packages

The unpublished https://github.com/emacs-magus/once looks like it will be much more general.

I'm not opposed to letting that take over the name "once", eventually! https://github.com/emacs-magus/once/issues/9

To avoid namespace conflict, you can early-adopt that and keep your =once-hook= / =once-load= forms mostly functional like this:

#+BEGIN_SRC elisp
(use-package once
  :vc (:url "https://github.com/emacs-magus/once" :rev :newest)
  :demand
  :config
  (defun once-hook (hook function &optional _depth _local)
    "Unlike `add-hook', DEPTH and LOCAL ignored."
    (once (list :hooks hook) function))

  (defun once-load (feature function)
    (once-after-load feature function))
